...
static UINT16 btif_media_task_get_sbc_rate(void)
{
    ...
    if (!btif_av_is_peer_edr())
    {
        ...
        APPL_TRACE_ERROR("non-edr a2dp sink detected, restrict SBC rate to %d", rate);
...
tBTIF_STATUS btif_a2dp_setup_codec(tBTA_AV_HNDL hdl)
{
    ...
    /* for now hardcode 44/48 khz 32 bit stereo */
...
static void btif_media_task_avk_handle_timer(UNUSED_ATTR void *context)
{
    ...
    else
    {
        ...
        do
        {
            p_msg = (tBT_SBC_HDR *)fixed_queue_try_peek(btif_media_cb.RxSbcQ);
            if (p_msg == NULL)
                return;
...
static void btif_media_task_enc_update(BT_HDR *p_msg)
{
    ...
    else
    {
        ...
        /* Set the initial target bit rate to SBC HD*/
    ...
    switch (btif_media_cb.TxTranscoding)
    {
        case BTIF_MEDIA_TRSCD_PCM_2_SBC:
        {
            ...
            if(btif_av_is_peer_edr())
            {
                ...
                {
                    APPL_TRACE_ERROR("Error: tx_sbc_frames not updated, update from here");
                    btif_media_cb.tx_sbc_frames = check_for_max_number_of_frames_per_packet();
                }
...
static void btif_media_aa_prep_2_send(UINT8 nb_frame)
{
    ...
    {
        APPL_TRACE_WARNING("%s() - TX queue buffer count %d/%d", __func__,
            fixed_queue_length(btif_media_cb.TxAaQ), MAX_OUTPUT_A2DP_FRAME_QUEUE_SZ - nb_frame);
    }

    while (fixed_queue_length(btif_media_cb.TxAaQ) > (MAX_OUTPUT_A2DP_FRAME_QUEUE_SZ - nb_frame))
    {
        p_buf = fixed_queue_try_dequeue(btif_media_cb.TxAaQ);
        if (p_buf)
        {
            GKI_freebuf(p_buf);
        }
